		...
		if($_SESSION['droits']>=2){
			if(!isset($_POST['submit'])){
				...
				if($data['erreursSaisie']!=[]){
					...
					Socket::store('tablette','update','constructeur',$constructeur);
					header('Location: ./?r=constructeursModeles/afficherConstructeur&constructeur='.$_GET['constructeur']);
				}
			}
		}else{
				...
				if($_GET['ajout']=="constructeur"){
					if(isset($_POST['submit'])){
						...
						if($data['erreursSaisie']!=[]){
							...
							Socket::store('tablette','insert','constructeur',$constructeur);
							$this->afficher();
						}
					}else{
						if(!isset($_POST['cancel'])){
				...
				elseif($_GET['ajout']=="modele"){
					if(isset($_POST['submit'])){
						...
						if($data['erreursSaisie']!=[]){
							...
							Socket::store('tablette','insert','modele',$modele);
